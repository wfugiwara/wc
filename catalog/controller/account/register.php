...
class ControllerAccountRegister extends Controller {
	...
	public function estado_autocompletar() {
  	...
  	protected function validate() {
		...
		if (strlen(utf8_decode($this->request->post['confirm_email'])) < 1) {
			$this->error['confirm_email'] = "O campo Confirmação de e-mail é obrigatório";  	
		}
		...
		/*
		if (!$this->customer->renavam($this->request->post['placa1'])){
			$this->error['placa1'] = "O campo Renavam é inválido";
		}
		
		if ((strlen(utf8_decode($this->request->post['placa1'])) < 1)) {
			$this->error['placa1'] = "O campo Renavam do veiculo é obrigatório";
		}
		*/
