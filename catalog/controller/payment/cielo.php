...
class ControllerPaymentCielo extends Controller {
	...
	public function processar() {
		...
		/* Lê dados do $_POST */
		$Pedido->formaPagamentoBandeira = ($_POST["bandeira"] == 'visae' ? 'visa' : $_POST["bandeira"]);
		...
		if (isset($objResposta->$urlAutenticacao)) {
			$resp['success'] = 'success';
			$resp['url'] = (string)($objResposta->$urlAutenticacao);
		} else {
			$resp['error'] = 'error';
		}
	...
	function parcelamento() {
		...
		if ($bandeira != 0 || $bandeira != 'visae') {
			...
			?>
				...
				<input type="radio" id="formaPagamento" name="formaPagamento" value="1" checked="checked"><?php echo '1x de '. number_format($valor, 2, ',', '.') .' sem juros.';?>
				<?php
			...
			?>
				...
				for ($p = 2; $p <= $maximo_parcelas; $p++) {
				?>
					<br><input type="radio" id="formaPagamento" name="formaPagamento" value="<?php echo $p;?>"><?php echo $p .'x (a consultar)';?>
				<?php
				}
			}
		...
		?>
			...
			<input type="radio" id="formaPagamento" name="formaPagamento" value="A" checked="checked"><?php echo '1x de '. number_format($valor, 2, ',', '.') .'<br /><br />';?>
		<?php
		}
	}
