...
<?php if ($addresses) { ?>
<input type="radio" name="<?php echo $type; ?>_address" value="existing" id="<?php echo $type; ?>-address-existing" checked="checked"  data-theme="a"/>
...
<?php } ?>
<p>
<input type="radio" name="<?php echo $type; ?>_address" value="new" id="<?php echo $type; ?>-address-new"  data-theme="a"/>
...
<select name="country_id" class="large-field" onchange="$('#<?php echo $type; ?>-address select[name=\'zone_id\']').load('index.php?route=checkout/address/zone&country_id=' + this.value);" data-native-menu="true"  data-theme="a">
