  ...
  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" data-ajax="false">
    ...
    <h2 style="margin:5px;"><?php echo $text_your_address; ?> </h2>
      ...
      <input type="text" name="city" <?php if ($city) { ?> value="<?php echo $city; ?>" <?php } ?> placeholder="*<?php echo $entry_city; ?>"/>
            ...
            <?php } ?>
      <input type="text" name="postcode" <?php if ($postcode) { ?> value="<?php echo $postcode; ?>" <?php } ?>placeholder="*<?php echo $entry_postcode; ?>"/>
            <?php if ($error_postcode) { ?>
            <div class="error"><?php echo $error_postcode; ?></div>
            <?php } ?><br />
             <span class="required">*</span> <?php echo $entry_country; ?>
         <select name="country_id" onchange="$('select[name=\'zone_id\']').load('index.php?route=account/register/zone&country_id=' + this.value + '&zone_id=<?php echo $zone_id; ?>');" data-native-menu="true"  data-theme="a">
    ...
    <fieldset data-role="controlgroup">
	   <legend id="text_agree"><?php echo $text_agree; ?></legend>
        <?php if ($agree) { ?>
        <input type="checkbox" name="agree" value="1" checked="checked" for="agree"  data-theme="a"/>
